    ...
    public void process(final ChannelHandlerContext ctx, final C2SAskCreateCharPacket packet) {
        ...
        if (slot > -1) {
            databaseProxyConnector.createCharacter(
                    ...
                    slot);
            CharacterDTO characterDTO = databaseProxyConnector.findCharacter(session.getUser().getAccount().getId(), slot);

            ctx.writeAndFlush(new S2CAnsCreateCharPacket(characterDTO));
        } else {
            //TODO: Error packet
